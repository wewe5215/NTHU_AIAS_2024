for i in range(5):
        for k in range(9):
            offset_A = 0x0 + 0x4 * i                    # A[0][i]
            offset_B_src = 0x0 + 0x90 * i + 0x4 * k     # B[i][k]
            offset_B_dst = 0x100 + 0x90 * i + 0x4 * k
            offset_C = 0x400 + 0x4 * k                  # C[0][k]
            assem = [] 
            print(f'## i = {i}, k = {k}')
            # ==========mat_A===============
            # DMA_REG_BASE_ADDR --------
            ...
            assem.append("sw t2, 0(t1)")
            # wait dma
            wait_dma_label = f'wait_dma1_{i}{k}'
            ...
            assem.append("sw t2, 0(t1)")
            # wait dma
            wait_dma_label = f'wait_dma2_{i}{k}'
            assem.append(f'{wait_dma_label}:')
            assem.append("la t6, DMA_OFFSET_DONE")
            assem.append("lw t1, 0(t6)")
            assem.append("add t1, t1, t0")
            ...
            assem.append("lw t1, 0(t6)")
            assem.append("add t1, t1, t0")
            assem.append("li t2, 0x00090905")
            assem.append("sw t2, 0(t1)")
            assem.append("li t1, 0x0")
            assem.append("add t1, t1, t0")
            assem.append("li t2, 0x00000001")
            assem.append("sw t2, 0(t1)")
            wait_sa = f'wait_sa_{i}{k}'
