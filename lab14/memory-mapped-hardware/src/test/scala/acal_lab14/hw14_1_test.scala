...
class top_Hw14_1_Test(dut: top_Hw14_1) extends PeekPokeTester(dut) with AXITester{
    ...
    for(i <- 0 until 4) {
        ...
        val req = genAXIAddr(BigInt(Hw14_1_Config.Mem_Base_ADDR + 4 * i))
    ...
    for(i <- 0 until 4) {
        if(i==0)
            println("Matrix B :")
        poke(dut.io.tb_slave.r.ready, true)
        val req = genAXIAddr(BigInt(Hw14_1_Config.Mem_Base_ADDR + 4 * 4 + 4 * i))
        poke(dut.io.tb_slave.ar.valid, true)
        poke(dut.io.tb_slave.ar.bits.addr, req.addr)
        poke(dut.io.tb_slave.ar.bits.burst, req.burst)
        poke(dut.io.tb_slave.ar.bits.cache, req.cache)
        ...
        poke(dut.io.tb_slave.ar.bits.size, req.size)
        step(1)
        val addr = (Hw14_1_Config.Mem_Base_ADDR + 4 * 4 + 4 * i).toHexString
        while(!peek(dut.io.tb_slave.r.valid)) {
            step(1)
        }
        var value : String = ""
    ...
    for(i <- 0 until 4) {
        ...
        poke(dut.io.tb_slave.r.ready, true)
        val req = genAXIAddr(BigInt(Hw14_1_Config.Mem_Base_ADDR + 4 * 8 + 4 * i))
        poke(dut.io.tb_slave.ar.valid, true)
        poke(dut.io.tb_slave.ar.bits.addr, req.addr)
        poke(dut.io.tb_slave.ar.bits.burst, req.burst)
        poke(dut.io.tb_slave.ar.bits.cache, req.cache)
        ...
        poke(dut.io.tb_slave.ar.bits.size, req.size)
        step(1)
        val addr = (Hw14_1_Config.Mem_Base_ADDR + 4 * 8 + 4 * i).toHexString
        while(!peek(dut.io.tb_slave.r.valid)) {
            step(1)
        }
        var value : String = ""
...
object top_Hw14_1_Test extends App {
  Driver.execute(
    args,
    () =>
      new top_Hw14_1()
  ) { c: top_Hw14_1 =>
    new top_Hw14_1_Test(c)
  }
}
