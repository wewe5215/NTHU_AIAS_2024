...
class top(idWidth: Int, addrWidth: Int, dataWidth: Int, instrBinaryFile: String) extends Module {
    ...
    val cpu = Module(new PiplinedCPU(32,32))
    ...
    when(mState === mReadSend) {
      ...
      io.master.ar.bits.addr := Cat(0.U(16.W), cpu.io.DataMem.raddr)
    ...
    when(mState === mWriteSend) {
      ...
      // base address = n * stride
      io.master.aw.bits.addr := Cat(0.U(16.W), cpu.io.DataMem.waddr)
