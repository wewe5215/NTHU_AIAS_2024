...
class SA(rows: Int, cols: Int, addr_width: Int, data_width: Int, reg_width: Int) extends Module {
  ...
  val read_accumulated = RegInit(0.U(3.W))
  val reading_accumulated = RegInit(false.B)

  // use for select partial read data
  val word_readData = Wire(UInt(data_width.W))
  val accumulated_mat_C = RegInit(VecInit(Seq.fill(4)(0.U(data_width.W))))
  ...
  io.raddr := Mux(
      ...
      b_base_addr + ((weight_cnt - 1.U) << 2) * io.mmio.MAT_MEM_STRIDE(15,8), // todo : b_base_addr + ((weight_cnt - 1.U) << 2) * mmio.stride(15,8)
      Mux(input_cnt >= 4.U || stateReg === sCheck,
          c_base_addr + (read_accumulated << 2) * io.mmio.MAT_MEM_STRIDE(23,16),
          a_base_addr + (input_cnt << 2) * io.mmio.MAT_MEM_STRIDE(7,0))
  )
  io.waddr := c_base_addr + (output_cnt << 2) * io.mmio.MAT_MEM_STRIDE(23,16)
  ...
  }.reduce(_ + _)) + accumulated_mat_C(output_cnt)
  ...
  when(stateReg === sStall_1 || stateReg === sPreload) {
    ...
    when(input_cnt + 1.U === 4.U){
      reading_accumulated := true.B
    }
    ...
    input_cnt  := 0.U
    output_cnt := 0.U
    read_accumulated := 0.U
  }.otherwise {
    // DontCare
  }

  when(reading_accumulated){
    read_accumulated := read_accumulated + 1.U
    when(read_accumulated >= 1.U){
      accumulated_mat_C(read_accumulated - 1.U) := word_readData
    }
    when(read_accumulated + 1.U === 5.U){
      reading_accumulated := false.B
    }
  }
}
