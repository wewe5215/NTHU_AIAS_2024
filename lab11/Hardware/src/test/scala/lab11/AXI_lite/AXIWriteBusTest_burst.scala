...
class AXIWriteBusBurstTest extends AnyFlatSpec with ChiselScalatestTester with AXITester{
    ...
    "Master" should "Write data to each slave according to different address" in {
        test(new AXIWriteBus(
            ...
            fork{
                dut.io.slave(0).writeAddr.expectDequeue(genAXIAddr_burst(BigInt("9000", 16), 1, 1, 1))
                dut.io.slave(0).writeAddr.expectDequeue(genAXIAddr_burst(BigInt("9008", 16), 2, 2, 2))
            }.fork{
                dut.io.slave(1).writeAddr.expectDequeue(genAXIAddr_burst(BigInt("19000", 16), 45, 1, 4))
                dut.io.slave(1).writeAddr.expectDequeue(genAXIAddr_burst(BigInt("19008", 16), 35, 2, 128))
            }.fork{
                ...
                dut.io.master.writeAddr.enqueue(genAXIAddr_burst(BigInt("9000", 16), 1, 1, 1))
                dut.io.master.writeAddr.enqueue(genAXIAddr_burst(BigInt("9008", 16), 2, 2, 2))
                dut.io.master.writeAddr.enqueue(genAXIAddr_burst(BigInt("19000", 16), 45, 1, 4))
                dut.io.master.writeAddr.enqueue(genAXIAddr_burst(BigInt("19008", 16), 35, 2, 128))
            }.fork{
