...
class top_SoCTest(dut: top_SoC) extends PeekPokeTester(dut) {
  ...
  while (!peek(dut.io.Hcf)) {
    ...
    var DMA_master_state = peek(dut.io.DMA_master_state).toInt
    var DMA_Rstate = peek(dut.io.DMA_Rstate).toInt
    var DMA_Wstate = peek(dut.io.DMA_Wstate).toInt
    var wData_DMA = (peek(dut.io.wData_DMA).toInt.toHexString).replace(' ', '0')
    var data_written = peek(dut.io.data_written).toInt
    var now_read_state_global = peek(dut.io.now_read_state_global).toInt
    var now_write_state_global = peek(dut.io.now_write_state_global).toInt
    var now_read_state_local = peek(dut.io.now_read_state_local).toInt
    var now_write_state_local = peek(dut.io.now_write_state_local).toInt
    var is_last = peek(dut.io.is_last_out).toInt
    // var len = peek(dut.io.len).toInt
    // var enable_out = (peek(dut.io.enable_out).toInt.toHexString).replace(' ', '0')
    // var source_info_out = (peek(dut.io.source_info_out).toInt.toHexString).replace(' ', '0')
    // var dest_info_out = (peek(dut.io.dest_info_out).toInt.toHexString).replace(' ', '0')
    // var size_cfg_out = (peek(dut.io.size_cfg_out).toInt.toHexString).replace(' ', '0')
    // var done_out = (peek(dut.io.done_out).toInt.toHexString).replace(' ', '0')
    println(
    ...
    println(s"[now_read_state_global ] ${"%2d".format(now_read_state_global)} [now_write_state_global] ${"%2d".format(now_write_state_global)}")
    println(s"[now_read_state_local ] ${"%2d".format(now_read_state_local)} [now_write_state_local] ${"%2d".format(now_write_state_local)}")
    println(s"[wData_DMA ] ${"%8s".format(wData_DMA)} [data_written] ${"%2d".format(data_written)}")
    println(s"[DMA_master_state ] ${"%2d".format(DMA_master_state)} [DMA_Rstate] ${"%2d".format(DMA_Rstate)} [DMA_Wstate] ${"%2d".format(DMA_Wstate)}")
    println(s"[is_last] ${"%1d".format(is_last)}")
    // println(s"[len ] ${"%8d".format(len)}, [is_last] ${"1d".format(is_last_out)}")
    // println(
    //     s"[enable_out]${"%8s".format(enable_out)} [source_info_out] ${"%8s".format(source_info_out)} " +
    //     s"[dest_info_out]${"%8s".format(dest_info_out)} [size_cfg_out]${"%8s".format(size_cfg_out)}" +
    //     s"[done_out]${"%8s".format(done_out)}"
    // )
    println("==============================================")
