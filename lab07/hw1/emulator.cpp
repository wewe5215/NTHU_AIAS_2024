...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			//instruction added
			case MUL: case MULHU: case REM: case REMU: case MULHSU:
	...
	while(!dexit) {
		...
		switch (i.op) {
			...
			case MULHU: rf[i.a1.reg] = (int32_t)(((uint64_t)rf[i.a2.reg] * (uint64_t)rf[i.a3.reg]) >> 32); break;
      		case MULHSU: rf[i.a1.reg] = (int32_t)(((int64_t)rf[i.a2.reg] * (uint64_t)rf[i.a3.reg]) >> 32); break;
			case REM:
				// overflow
		...
		if ( stepping ) {
			// printf("cur = %02d\n", cur.a1.reg);
			for ( int i = 0; i < 32; i++ ) {
				if ( rf[i] != rf_mirror[i] || cur.a1.reg == i ) 
					printf( ">> rf[x%02d] %d -> %d\n", i, rf_mirror[i], rf[i] );
				rf_mirror[i] = rf[i];
			}
		}
